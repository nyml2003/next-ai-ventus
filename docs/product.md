...
**文章目录命名规则**：
- 目录名格式：`{YYYY-MM}-{slug}`，如 `2024-06-hello-world`
- `slug` 是 URL 友好的短链接，支持自定义（如 `hello-world`）
- 同一 slug 不能重复（全局唯一），重复时提示修改
...
|------|-----|---------|--------|
| **登录页** | `/login` | JWT 登录 | P0 |
| **管理首页** | `/admin` | 仪表盘（文章统计、快捷入口） | P1 | 可跳过，直接进入文章管理 |
