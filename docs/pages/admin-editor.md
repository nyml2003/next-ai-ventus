...
**说明**：编辑器页面通常隐藏 Sidebar，最大化写作区域

可选方案：
- **方案 A**：保留 Sidebar（BFF: `adminSidebar`），但默认折叠
...
**数据说明**：
- 编辑文章时，通过 `editor` BFF 模块获取文章内容
- 新建文章时，无 BFF 数据，显示 placeholder
...
**BFF 模块：`editorSettings`**（首屏模块，提供标签列表等）

**功能：**
- 文章元信息管理
- 标签列表数据来源：BFF `editorSettings` 模块
